...
object DisableFallout {
  ...
  def apply(fileName: String) = new DisableFallout(fileName)
}

/**
 * Disable Fallout for orders listed in a CSV file by updating TMOOMST and changing the fallout flag (TMFOFLG) to "Y".
 *
 * If the county keyed in the CSV file is not matched, a warning is generated.  If the
 * order is not found in TMOOMST or the fallout flag (OMFOFLG) is not enabled ("Y") a
 * warning is generated.  Finally, if updating the record fails, an error is generated.
 *
 * @param filename the path to the CSV file
 */
class DisableFallout(fileName: String) extends Logging {
  ...
  for (csvOrder <- csvEntries) {
    csvOrder county match {
      ...
      case Some(x) => {
        disableFallout(csvOrder.order, x)
      }
    }
  }

  /**
   * Disable Fallout.  The companyCode is globably defined in the package object becuase this version
   * of the progrm is specific to Gracy Title.
   *
   * @param orderNumber the Order Number
   * @param cntyCode the TIMS County Code
   */
  private def disableFallout(orderNumber: String, cntyCode: String) {
    logger debug ("Attempting to Disable Fallout: " + orderNumber + "," + companyCode + "," + cntyCode)

    // Format the order number for TIMS
    val fmtOrderNum = DisableFallout.formatOrderNum(orderNumber)
    ...
    records match {
      ...
      case Some(listRecords) => {
        for (record <- listRecords) {
          ...
          if (status == "Y") { // Attempt to disable fallout
            record.setField("OMFOFLG", "N")
            ...
            success match { // Error handling in case the update fails
              ...
              case Some(x) => logger info ("Disabled Fallout for Order: " + orderNumber + "," + companyCode + "," + cntyCode)
            }
